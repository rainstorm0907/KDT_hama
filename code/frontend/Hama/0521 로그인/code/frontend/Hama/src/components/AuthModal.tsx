  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center gap-2 mb-6">
        {(['basic', 'terms', 'personal'] as SignUpStep[]).map((s, i) => (
          <div
            key={s}
            className={`h-1.5 rounded-full transition-all ${
              s === step ? 'w-6 bg-black' : i < ['basic','terms','personal'].indexOf(step) ? 'w-3 bg-gray-400' : 'w-3 bg-gray-200'
            }`}
          />
        ))}
      ...
      {step === 'personal' && (
        <>
          ...
          <form className="w-full space-y-4" onSubmit={handlePersonalSubmit}>
            ...
            <button
              disabled={!birthDate || !phone || !nickname}
              className={`mt-2 w-full rounded-2xl py-4 font-bold transition-all ${hairline.primaryButton} ${hairline.focus} disabled:opacity-40 disabled:cursor-not-allowed`}
            >
              시작하기
            </button>
...
function NaverLoginButton({ label }: { label: string }) {
  return (
    <a
      ...
      className={`flex w-full items-center justify-center gap-3 rounded-2xl py-4 font-bold text-white transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#03C75A] active:brightness-90`}
