...
from opensearch.documents import (
    MAX_PLAUSIBLE_PRICE,
    MIN_PLAUSIBLE_PRICE,
    SUMMARY_PRICE_FLOOR_RATIO,
)
...
def search_summary(products: list[dict[str, object]]) -> dict[str, object]:
    ...
    # 교환/광고 글의 플레이스홀더 가격(1원·500원, 999,999,999원)은 시세 요약에서 제외한다.
    # 결과 중앙값 대비 비정상 저가도 낚시·잡글로 보고 제외하며,
    # 전부 걸러지면(초저가 카테고리) 원본 표본으로 폴백한다.
    prices = [
        price
        for price in all_prices
        if MIN_PLAUSIBLE_PRICE <= price < MAX_PLAUSIBLE_PRICE
    ] or all_prices
    if len(prices) >= 5:
