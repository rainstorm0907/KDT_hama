...
DEFAULT_RESULTS_DIR = PYTHON_DIR / "crawling" / "results" / "by_keyword"
DEFAULT_PREVIEW_CSV = PYTHON_DIR / "analysis" / "handoff" / "keyword_db_input_df.csv"
BATCH_SIZE = 500
T = TypeVar("T")
...
def find_csv_paths(args: argparse.Namespace) -> list[Path]:
    ...
    if not csv_paths:
        csv_paths = sorted((PYTHON_DIR / "crawling" / "results").glob("*.csv"))
...
def fetch_item_ids(database, item_payloads) -> dict[tuple[str, str], int]:
