...
from cluster_by_platform_tokens import load_token_blacklist, run_clustering, title_has_blacklisted_token
...
token_blacklist_file = base_dir / "blacklist_tokens.csv"
...
def should_exclude_by_blacklist(title):
    if contains_blacklist_keyword(title):
        return True
    return title_has_blacklisted_token(title, token_blacklist)
...
if all_dfs:
    ...
    if RUN_CLUSTERING_AFTER_CRAWL:
        ...
        print(f"클러스터링 요약 결과(L2 리프 단위): {cluster_outputs['summary_output_path']}")
        print(f"클러스터링 트리 결과: {cluster_outputs['tree_output_path']}")
