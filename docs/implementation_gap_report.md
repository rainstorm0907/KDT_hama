...
5. `preprocessing/` 구현 및 분석 결과를 적재 필터에 반영
...
- `code/backend/src/main/python/preprocessing`은 아직 `.gitkeep`만 있습니다.
- 중복 상품 제거, 가격/날짜/상태값 정규화, DB 저장 전 검증 스크립트가 없습니다.
- `HamaCollectionPipeline`의 중고나라/번개장터 fetch 함수는 TODO 상태이며 빈 리스트를 반환합니다.
- ML 카테고리 분류기는 stub 상태입니다.

### 블랙리스트 / 분석 반영

- `blacklist_keywords.csv`, `blacklist_tokens.csv`는 있지만 실제 크롤링/적재 코드에 연결되어 있지 않습니다.
- 분석 결과인 `price_outliers`, `title_keyword_accuracy`, `bracket_clusters`가 운영 필터나 Supabase 적재에 아직 반영되지 않았습니다.
- 가격 이상치 분석 노트북의 필터 조건 일부는 노트북 내부 하드코딩입니다.
- 분석 기준 CSV가 시점별로 섞여 있어 재현성 검증이 필요합니다.

### Supabase

- Python 적재는 `platforms`, `items`, `price_history` 중심입니다.
...
- `apply_supabase_schema.py`는 migration 이력 테이블 없이 SQL 파일 전체를 실행합니다.
- `import_csv_to_supabase.py`는 전처리 파이프라인을 거치지 않고 CSV를 적재할 수 있습니다.
